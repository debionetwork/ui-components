<template lang="pug">
  .ui-debio-dropdown(:class="classes" :style="computeStyle" v-click-outside="handleBlur")
    label.ui-debio-dropdown__label(:for="uuid" @click="openOptions" :aria-label="label" v-if="label") {{ label }}
...
export default {
  ...
  computed: {
    classes() {
      return [
        ...
        { "ui-debio-dropdown--disabled": this.disabled },
  ...
  methods: {
    ...
    openOptions() {
      if ((this.closeOnSelect && this.active && !this.searchFocus) || this.disabled) this.active = false
